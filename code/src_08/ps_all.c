...
/*   Updated: 2023/06/29 19:07:36 by antoda-s         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_cost_b(t_stack *h, t_elem *node)
{
	...
	else if (cost == (mid) && (node->move_b == RX || node->move_b == RRX) && !(h->size_b % 2))
	//else if (cost == (h->size_b / 2 + h->size_b % 2) && (node->move_b == RX || node->move_b == RRX) && !(h->size_b % 2))
		node->move_b = node->move_b;
...
int		ft_cost_a(t_stack *h, t_elem *node)
{
	...
	else if (cost == (mid) && (node->move_b == RX || node->move_b == RRX) && !(h->size_a % 2))
...
void	ft_cost_src_x(t_elem *node, int range, char stack_src)
{
	...
	while (i >= -range)
	{
		...
		ft_cost_src_save(cur, move, cost, stack_src);
		i--;
	}
	return ;
}
