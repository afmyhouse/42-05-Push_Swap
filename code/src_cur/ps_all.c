...
int	ft_ss(t_stack *h)
{
	if (_SHOWFUNCTION)
		...
		, YLW, WTH, RED, h->ta, WTH, RED, h->tb, WTH);
	ft_sx(h, 'a', _NOPRINT);
	ft_sx(h, 'b', _NOPRINT);
	write(1, "ss\n", 3);
	h->moves++;
...
void	ft_cost_analitics(t_stack *h, t_elem *head)
{
	int	raw_cost;
	int	rr_cost;
	int	rrr_cost;

	if (head->move_a == 'F'  && head->move_b == 'R')
	{
		rr_cost = ft_max_int(head->cost_a, h->size_b - head->cost_b);
		rrr_cost = ft_max_int(h->size_a - head->cost_a, head->cost_b);
		raw_cost = head->cost_a + head->cost_b;
		if (rr_cost <= rrr_cost && rr_cost <= raw_cost)
			ft_cost_src_save(head, 'F', h->size_b - head->cost_b, 'b');
		else if (rrr_cost <= rr_cost && rrr_cost <= raw_cost)
			ft_cost_src_save(head, 'R', h->size_a - head->cost_a, 'a');
	}
	else if (head->move_a == 'R' && head->move_b == 'F')
	{
		rr_cost = ft_max_int(h->size_a - head->cost_a, head->cost_b);
		rrr_cost = ft_max_int(head->cost_a, h->size_b - head->cost_b);
		raw_cost = head->cost_a + head->cost_b;
		if (rr_cost <= rrr_cost && rr_cost <= raw_cost)
			ft_cost_src_save(head, 'F', h->size_a - head->cost_a, 'a');
		else if (rrr_cost <= rr_cost && rrr_cost <= raw_cost)
			ft_cost_src_save(head, 'R', h->size_b - head->cost_b, 'b');
	}
	return ;
}
...
t_elem	*ft_get_best(t_stack *h, char stack_src)
{
	...
	if (_TEST)
		ft_cost_analitics(h, best);
	i--;
	while (i >= -range)/************ALTERADO******************/
	{
		if (_TEST)
			ft_cost_analitics(h, ft_get_location(cur, i));
...
int	main(int ac, char **av)
{
	...
	if (ac == 1)
	{
		ac = 2;
		/* 30 */
		av[1] = "21 13 30 3 18 26 1 11 27 10 19 29 6 8 9 20 22 24 2 5 14 16 23 25 4 17 28 15 7 12";
		/* 30 */
	}
	else if (ac == 2)
	{
		if (atoi(av[1]) == 9)
			...
			/* 100 */
			av[1] = "86 16 6 69 93 23 12 70 52 28 47 22 40 60 24 10 95 44 65 100 3 63 51 82 71 39 17 5 33 43 53 34 89 27 37 2 32 50 98 57 1 46 21 64 8 45 18 14 13 15 25 76 79 91 9 81 85 4 11 68 59 55 7 84 73 66 87 19 92 29 67 42 90 77 80 54 38 20 35 83 62 49 30 75 31 74 58 36 26 72 56 61 99 88 94 48 96 78 41 97";
			/* 100 */
		else
			/* 30 */
			av[1] = "21 13 30 3 18 26 1 11 27 10 19 29 6 8 9 20 22 24 2 5 14 16 23 25 4 17 28 15 7 12";
